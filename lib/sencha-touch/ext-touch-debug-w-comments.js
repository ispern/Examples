    ...
    onComplete : function(r) {
        ...
        if (status >= 200 && status < 300) {
            response = this.createResponse(r);
                        this.fireEvent('requestcomplete', response);
            if (options.success) {
                if (!options.scope) {
            ...
            this.fireEvent('requestexception', response);
            if (options.failure) {
                if (!options.scope) {
                    options.failure(response);
                }
                else {
                    options.failure.call(options.scope, response);
                }
            }
        }
...
Ext.data.Model = Ext.extend(Ext.util.Observable, {
    ...
    constructor: function(data, id) {
        ...
        Ext.data.Model.superclass.constructor.apply(this, arguments);
        ...
        this.id = (id || id === 0) ? id : Ext.data.Model.id(this);
        this.data = data;
...
Ext.Component = Ext.extend(Ext.util.Observable, {
    ...
    */
     ...
     * Acceptable values are: 'horizontal', 'vertical' and false
     ...
     * @param config {Mixed}
     * Acceptable values are a Ext.Scroller configuration, 'horizontal', 'vertical' and false
     */
    ...
    /**
     * Show the component.
     * @param {Boolean} animation (optional) Defaults to false.
     */
...
Ext.SplitButton = Ext.extend(Ext.Container, {
     ...
     * @returns {Array/Button} A button or an array of buttons (if allowMultiple is true)
     */
    getActive : function() {
    ...
    setActive : function(btn) {
        ...
        if (this.allowMultiple) {
            ...
            if (btn) {
                if (!this.activeButtons.indexOf(btn)) {
...
Ext.Panel = Ext.extend(Ext.Container, {
    ...
    renderTpl: new Ext.XTemplate(
        '<div <tpl if="id">id="{id}"</tpl> class="{baseCls} {cls} {cmpCls}<tpl if="ui"> {uiBase}-{ui}</tpl>">'+
...
Ext.List = Ext.extend(Ext.DataView, {
    ...
    onScroll : function(scroller, pos, options) {
        if (!scroller.animating) {
            ...
            if (headerY != 0) {
                if (options.animate && !Ext.platform.isAndroidOS) {
                    new Ext.Anim({
                        from: {
                            '-webkit-transform': 'translate3d(0px, 0px, 0px)'
                        },
                        to : {
                            '-webkit-transform': 'translate3d(0px, ' + headerY + 'px, 0px)'
                        },
                        autoClear: false
                    }).run(header);
                }
        ...
        else if (this.activeHeader) {
            this.activeHeader.setStyle('-webkit-transform', null);
            scroller.on('scrollend', this.onScroll, this, {single: true, animate: true});
        }
    },
    ...
    collectData : function(records, startIndex) {
        ...
        for (i = 0, ln = groups.length; i < ln; i++) {
            ...
            results.push({
                ...
                items: this.itemTpl.apply(group.children)
            });
        }
        ...
        for (i = 0, ln = this.slots.length; i < ln; i++) {
            ...
            slotItem = {
                ...
                scroll: {
                    ...
                    scrollbars: false,
                    snapping: true,
                    // snapY cannot be determined until render time.
                    deceleration: 0.006,
                    friction: 0.2,
...
Ext.form.FormPanel = Ext.extend(Ext.Panel, {
    ...
    renderTpl: new Ext.XTemplate(
        '<form <tpl if="id">id="{id}"</tpl> class="{baseCls} {cls} {cmpCls}<tpl if="ui"> {uiBase}-{ui}</tpl>">'+
            '<div class="{baseCls}-body"<tpl if="bodyStyle"> style="{bodyStyle}"</tpl></div>'+
        '</form>',
        {compiled: true}
    ),
...
Ext.form.Slider = Ext.extend(Ext.form.Field, {
    ...
    /**
     * Sets the new value of the slider, constraining it withing {@link minValue} and {@link maxValue}, and snapping to the nearest
